...
at::Tensor bvh_forward(at::Tensor triangles, int max_collisions = 16, double t = 0) {
//at::Tensor bvh_forward(at::Tensor triangles, int max_collisions = 16) {
    ...
    bvh_cuda_forward(triangles,
            &collisionTensor, max_collisions, t);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("forward", &bvh_forward, "BVH collision forward (CUDA)",
        py::arg("triangles"), py::arg("max_collisions") = 16, py::arg("t") = 0.);
}
